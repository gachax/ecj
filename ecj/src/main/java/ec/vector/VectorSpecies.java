...
public class VectorSpecies extends Species
    ...
    private class ParameterLoader implements Callable {
        ...
        ParameterLoader (final EvolutionState state, final Parameter base, Parameter def, int startIndex, int endIndex) {
            this.state = state;
            this.base = base;
            ...
            this.def = def;
            try {
                this.parameters = new ParameterDatabase(state.parameters.getFile());
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
        ...
        public Object call() throws Exception {
            ...
            for (int x = startIndex; x <= endIndex; x++) {
                loadParametersForGeneNoSync(state, x, base, def, "" + x, parameters);
            }
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        int geneCountToProcess = 400;
        int threadCount = genomeSize/geneCountToProcess;
        if (threadCount == 0) {
            threadCount = 1;
        }

        ExecutorService parameterLoaderExecutorService = Executors.newFixedThreadPool(threadCount);
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            int startIndex = 0;
            if (i > 0) {
                startIndex = geneCountToProcess * i;
            }

            int endIndex = 0;
            if (i == threadCount - 1) {
                endIndex = genomeSize - 1;
            } else {
                endIndex = startIndex + geneCountToProcess - 1;
            }

            tasks.add(parameterLoaderExecutorService.submit(new ParameterLoader(state, base, def, startIndex, endIndex)));
        ...
        parameterLoaderExecutorService.shutdown();
    ...
    */
    ...
    protected void loadParametersForGeneNoSync(EvolutionState state, int index, Parameter base, Parameter def, String postfix, ParameterDatabase parameters)
    {
        ...
        if (parameters.exists(base.push(P_MUTATIONPROB).push(postfix), def.push(P_MUTATIONPROB).push(postfix)))
        {
            mutationProbability[index] = parameters.getDoubleWithMax(base.push(P_MUTATIONPROB).push(postfix), def.push(P_MUTATIONPROB).push(postfix), 0.0, 1.0);
            if (mutationProbability[index] == -1.0)
                state.output.fatal("Per-gene or per-segment mutation probability must be between 0.0 and 1.0 inclusive",
                        base.push(P_MUTATIONPROB).push(postfix),def.push(P_MUTATIONPROB).push(postfix));
        }

        if (parameters.exists(base.push(P_DUPLICATE_RETRIES).push(postfix), def.push(P_DUPLICATE_RETRIES).push(postfix)))
        {
            duplicateRetries[index] = parameters.getInt(base.push(P_DUPLICATE_RETRIES).push(postfix), def.push(P_DUPLICATE_RETRIES).push(postfix));
            if (duplicateRetries[index] < 0)
                state.output.fatal("Duplicate Retries for gene " + index + ", if defined must be a value >= 0",
                        base.push(P_DUPLICATE_RETRIES).push(postfix), def.push(P_DUPLICATE_RETRIES).push(postfix));
        }
